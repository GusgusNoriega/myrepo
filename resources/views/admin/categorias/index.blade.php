...
<div id="cats-app" class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
  ...
  <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
    <div>
      ...
      <p class="text-sm text-gray-500">Gestiona categorías por negocio (jerarquía opcional con <em>parent</em>).</p>
    </div>
  ...
  <div class="relative m-0 w-full rounded-t-2xl bg-white p-4 shadow-xl sm:m-4 sm:w-[760px] sm:rounded-2xl">
    ...
    <form id="form-cat" class="grid grid-cols-1 gap-4 sm:grid-cols-2">
      ...
      <div class="sm:col-span-1">
        <label class="block text-sm font-medium text-gray-700">Business ID*</label>
        <input id="i_business_id" type="number" min="1" required
          class="mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
        <p class="mt-1 text-xs text-gray-500">El negocio al que pertenece.</p>
      </div>
      ...
      <div class="sm:col-span-1">
        ...
        <p class="mt-1 text-xs text-gray-500">Debe ser del mismo negocio.</p>
      </div>
      ...
      <div class="sm:col-span-1">
        ...
        <input id="i_slug" type="text" placeholder="(opcional, se autogenera si se deja vacío)"
          class="mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
      ...
      <p id="edit-hint" class="sm:col-span-2 mt-1 hidden text-xs text-gray-500">
        Nota: en edición no se cambia el <strong>Business ID</strong>.
      </p>
    </form>
  </div>
</div>
...
(() => {
  ...
  function renderList() {
    tbodyDesktop.innerHTML = state.items.map(c => `
      <tr>
        ...
        <td class="px-4 py-3 text-sm text-gray-700">${escapeHtml(c.business?.name ?? ('ID ' + (c.business_id ?? '—')))}</td>
    ...
    cardsMobile.innerHTML = state.items.map(c => `
      <div class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
        ...
        <div class="mt-3 grid grid-cols-2 gap-2 text-sm text-gray-700">
          <div><span class="text-gray-500 text-xs">Negocio:</span> ${escapeHtml(c.business?.name ?? ('ID ' + (c.business_id ?? '—')))}</div>
  ...
  fBusiness.addEventListener('input', () => { state.filters.business_id = fBusiness.value.trim(); doFilter(); });
  ...
  function collectPayload(isEdit) {
    ...
    if (slugVal !== '') body.slug = slugVal; // si se deja vacío, el backend lo generará
  ...
  async function openEdit(id) {
    ...
    try {
      ...
      catId.value     = c.id;
      iBusiness.value = c.business_id ?? '';
      iParent.value   = c.parent_id ?? '';
      iName.value     = c.name ?? '';
      iSlug.value     = c.slug ?? '';
      iPosition.value = c.position ?? 0;

      iBusiness.disabled = true;  // no cambia en edición
      iAutoSlug.checked  = false; // no autogenerar en edición por defecto
